...
ACTIVE_REQUESTS = Gauge(
    ...
    "Current number of active inventory requests"
)
...
# Returns the route template (e.g. "/inventory/{product_id}") instead
# of the literal path, so metric labels don't grow unbounded per
# product_id.
...
@app.get("/inventory")
def list_inventory():
    log.info("listing_inventory", products=len(inventory))
    return {"inventory": inventory}


@app.get("/inventory/{product_id}")
def get_stock(product_id: str, request: Request):
    trace_id = request.headers.get("x-trace-id", str(uuid.uuid4())[:16])
    if product_id not in inventory:
        log.warning("product_not_found", product_id=product_id, trace_id=trace_id)
        raise HTTPException(status_code=404, detail="Product not found")
    return {"product_id": product_id, "stock": inventory[product_id]}


@app.put("/inventory/reduce")
def reduce_stock(req: StockUpdateRequest, request: Request):
    trace_id = request.headers.get("x-trace-id", str(uuid.uuid4())[:16])
    inv_log = log.bind(
        trace_id=trace_id,
    ...
    if random.random() < 0.08:
        ...
        inv_log.warning("slow_stock_check", delay_seconds=round(delay, 2))
        time.sleep(delay)

    pid = req.product_id
    if pid not in inventory:
        inv_log.error("product_not_found_for_reduction")
        raise HTTPException(status_code=404, detail="Product not found")

    if inventory[pid] < req.quantity:
        STOCK_REDUCTION_FAILURES.labels(product_id=pid).inc()
        inv_log.error(
            "insufficient_stock",
    ...
    STOCK_LEVEL.labels(product_id=pid).set(inventory[pid])
    ...
    # Low stock warning log
    if inventory[pid] < LOW_STOCK_THRESHOLD:
        LOW_STOCK_ALERTS.labels(product_id=pid).inc()
        inv_log.warning(
            "low_stock_warning",
            product_id=pid,
    ...
    if inventory[pid] == 0:
        inv_log.error("product_out_of_stock", product_id=pid)

    return {"product_id": pid, "remaining_stock": inventory[pid]}


@app.put("/inventory/restock")
def restock(req: StockUpdateRequest, request: Request):
    trace_id = request.headers.get("x-trace-id", str(uuid.uuid4())[:16])
    pid = req.product_id
    if pid not in inventory:
        inventory[pid] = 0
    inventory[pid] += req.quantity
    STOCK_LEVEL.labels(product_id=pid).set(inventory[pid])
    log.info("product_restocked", product_id=pid, new_stock=inventory[pid], trace_id=trace_id)
    return {"product_id": pid, "new_stock": inventory[pid]}